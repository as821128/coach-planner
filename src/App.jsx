...
function HomeView({ data, openSession }) {
  return (
    <div style={t.page}>
...
function SessionView({ student, session, setGoal, setNotes, setHomework, setNextFocus, setBodyField, showLib, setShowLib, data, addEx, updEx, delEx, moveEx, loading, generate, setView }) {
  ...
  return (
    <div style={t.page}>
      ...
      <div style={t.sec}>
        ...
        {session.exercises.map(ex=><ExRow key={ex.id} ex={ex} updEx={updEx} moveEx={moveEx} delEx={delEx} />)}
...
function ExportView({ output, syncing, copy, setView }) {
  return (
    <div style={t.page}>
...
function SettingsView({ data, setData, showToast, newName, setNewName, addStudent }) {
        ...
        <div style={{marginTop:"12px"}}>
          {data.students.map(s=>(
            <div key={s.id} style={t.card}>
              ...
              <button style={t.btnDanger} onClick={()=>setData(d=>({...d,students:d.students.filter(st=>st.id!==s.id)}))}>刪除</button>
            </div>
          ))}
        </div>
      </div>
...
export default function CoachPlanner() {
  ...
  const addStudent = useCallback(() => {
    setNewName(name => {
      ...
      setData(d=>({...d,students:[...d.students,{id:Date.now(),name:name.trim(),history:[]}]}));
      showToast("✅ 學生已新增");
      return "";
    });
  }, [showToast]);
  ...
  const generate = useCallback(async () => {
    setSession(currentSession => {
      if (!currentSession.exercises.length) { showToast("請先加入動作！"); return currentSession; }

      setLoading(true);
      const exLines = currentSession.exercises.map(e =>
        `${e.name}${e.weight?" "+e.weight+"kg × ":""}${e.reps}下 × ${e.sets}組${e.note?"（"+e.note+"）":""}`
      ).join("\n");
      const sheetExercises = currentSession.exercises.map(e=>e.name).join("、");
      const sheetWeights   = currentSession.exercises.map(e=>e.weight?`${e.name}${e.weight}kg`:"").filter(Boolean).join("、")||"—";

      setStudent(currentStudent => {
        const prompt = `你是一位專業健身教練助理。根據以下上課資訊，產出兩份文件。
【學生】${currentStudent.name}【日期】${currentSession.date}【今日目標】${currentSession.goal||"（未填）"}
【訓練動作】\n${exLines}
【教練觀察筆記】${currentSession.notes||"（未填）"}【回家作業】${currentSession.homework||"（未填）"}
【下次重點】${currentSession.nextFocus||"（未填）"}
【身體狀況】足弓:${currentSession.bodyStatus.foot} 膝蓋:${currentSession.bodyStatus.knee} 肩膀:${currentSession.bodyStatus.shoulder} 核心:${currentSession.bodyStatus.core} 疼痛分數:${currentSession.bodyStatus.pain}
...
{"studentReport":"給學生看的訓練紀錄（繁體中文、專業親切、含日期動作清單分析鼓勵）","goalSummary":"今日目標15字內","observationSummary":"動作觀察15字內","bodyIssueSummary":"身體問題15字內"}`;
        ...
        fetch("https://api.anthropic.com/v1/messages", {
          ...
          const row = [
            currentSession.date, currentStudent.name, parsed.goalSummary, sheetExercises, sheetWeights,
            parsed.observationSummary, parsed.bodyIssueSummary,
            currentSession.homework||"", currentSession.nextFocus||"",
            currentSession.bodyStatus.foot, currentSession.bodyStatus.knee,
            currentSession.bodyStatus.shoulder, currentSession.bodyStatus.core, currentSession.bodyStatus.pain
          ];
          const record = { date:currentSession.date, exercises:currentSession.exercises, goal:currentSession.goal,
            notes:currentSession.notes, bodyStatus:currentSession.bodyStatus, homework:currentSession.homework, nextFocus:currentSession.nextFocus };
          setData(prev=>({...prev,students:prev.students.map(s=>s.id===currentStudent.id?{...s,history:[...s.history,record]}:s)}));
          setOutput({ student:parsed.studentReport, sheet:row.join("\t"), row });
          setView("export");
          setSyncing(true);
          fetch(GAS_URL, { method:"POST", body:JSON.stringify({action:"appendRow",row}) })
            .then(()=>showToast("✅ 已自動寫入 Google Sheet！"))
  ...
  const copy = useCallback((text,label) => navigator.clipboard.writeText(text).then(()=>showToast(`✅ ${label} 已複製！`)), [showToast]);
  ...
  return (
    <div style={t.app}>
      ...
      {view==="home"    &&<HomeView data={data} openSession={openSession}/>}
      {view==="session" &&<SessionView student={student} session={session} setGoal={setGoal} setNotes={setNotes} setHomework={setHomework} setNextFocus={setNextFocus} setBodyField={setBodyField} showLib={showLib} setShowLib={setShowLib} data={data} addEx={addEx} updEx={updEx} delEx={delEx} moveEx={moveEx} loading={loading} generate={generate} setView={setView}/>}
      {view==="export"  &&<ExportView output={output} syncing={syncing} copy={copy} setView={setView}/>}
      {view==="history" &&<HistoryView data={data}/>}
      {view==="settings"&&<SettingsView data={data} setData={setData} showToast={showToast} newName={newName} setNewName={setNewName} addStudent={addStudent}/>}
